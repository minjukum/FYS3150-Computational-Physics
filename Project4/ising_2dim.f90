...
!        m_matrix(cycles) = abs(m)/(n_spins**2)
        ...
        IF ( cycles <= mcs*0.1) write(7,*) cycles, average(1)/(n_spins**2)/cycles, &
                                                   average(5)/(n_spins**2)/cycles
        IF (cycles >= 10000) write(8,*) cycles, average(1)/(n_spins**2)/cycles 
  ...
  DEALLOCATE(m_matrix)
  CALL analytical_values(temperature, n_spins)
...
SUBROUTINE analytical_values(temperature, n_spins)
  USE constants
  IMPLICIT NONE
  INTEGER, INTENT(in) :: n_spins
  REAL(DP), INTENT(in) :: temperature
  ...
  !WRITE(6,'(6F12.6)') temperature, ana_E/spin2, ana_M/spin2, ana_Cv/spin2, ana_X/spin2


END SUBROUTINE analytical_values
