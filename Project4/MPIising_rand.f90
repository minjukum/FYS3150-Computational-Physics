...
PROGRAM ising2dim
  ...
  USE mpi_constants
  USE f90library
  INTEGER :: n_spins, de, t, tstep, i, x, y
  ...
  INTEGER(DP) :: local_accepted, total_accepted
  ...
  DO t = 1, tstep
     ...
     total_accepted = 0; local_accepted = 0
     CALL initialize(n_spins, spin_matrix, E, M)
     CALL Metropolis(mcs, n_spins, my_rank, w, local_average, spin_matrix, local_accepted)
     ! collect all results
     CALL  MPI_REDUCE(local_average, total_average, 5, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierror)
     CALL  MPI_REDUCE(local_accepted,total_accepted, 1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierror)
     ! print results
     IF (my_rank == 0) THEN
        CALL WritetoFile(n_spins, mcs*numprocs, temperature, total_average, total_accepted)
     ENDIF
     ! update temp
...
!
SUBROUTINE  Metropolis(mcs, n_spins, my_rank, w, average, spin_matrix, accepted)
  USE constants
  IMPLICIT NONE
  INTEGER, INTENT(in) :: n_spins, my_rank, mcs
  INTEGER(DP), INTENT(INOUT):: accepted
...
!
SUBROUTINE WritetoFile(n_spins, mcs, temperature, average, accepted)
  USE constants
  IMPLICIT NONE
  ...
  WRITE(6,'(6F12.6)')temperature, Eaverage/n_spins/n_spins, &
                     Mabsaverage/n_spins/n_spins, &
                     Evariance/temperature/temperature, &
	             Mvariance/temperature
             

END SUBROUTINE WritetoFile

SUBROUTINE analytical_solution(temperature, n_spins)
  USE constants
  IMPLICIT NONE
  INTEGER, INTENT(in) :: n_spins
  REAL(DP), INTENT(in) :: temperature
  REAL(DP) :: Kb, beta, Z, ana_E, ana_E2, ana_M, ana_M2, ana_Cv, ana_X, spin2
  
  kb = 1.0
  beta = 1.0/(kb*temperature)
  spin2 = 2.0*2.0

  Z = 4.0*cosh(8*beta) + 12.0
  ana_E  = -32.0*sinh(8*beta)/Z
  ana_E2 = 256.0*sinh(8*beta)/Z
  ana_M  = (8.0*exp(8.0*beta) + 16.0)/Z
  ana_M2 = 32.0*(exp(8.0*beta) + 1)/Z
  ana_Cv = (ana_E2-ana_E*ana_E)/(kb*temperature*temperature)
  ana_X  = (ana_M2-ana_M*ana_M)/(kb*temperature)
  !WRITE(6,'(6F12.6)') ana_E/spin2, ana_M/spin2, ana_Cv/spin2, ana_X/spin2
