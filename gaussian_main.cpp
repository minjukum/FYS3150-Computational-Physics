...
int main(int argc, char const *argv[]) {
  ...
  //forward substitution(general)
  A_b_tilde(0) = A_b(0);
  g_tilde(1) = g(1);
  for (int i = 0; i < n-1; i++) {
    A_a_over_b_tilde(i) = A_a(i)/A_b_tilde(i);
    A_b_tilde(i+1) = A_b(i+1) - A_c(i)*A_a_over_b_tilde(i);
    g_tilde(i+2) = g(i+2) - g_tilde(i+1)*A_a_over_b_tilde(i);
  }

  //backward substitution(general)
  f(n) = g_tilde(n)/A_b_tilde(n-1);
  for (int i = n-1; i > 0; i--) {
    f(i) = (g_tilde(i) - A_c(i-1)*f(i+1)) / A_b_tilde(i-1);
  }
